...
export default function VentaBoleto() {
  ...
  const handleCantidadChange = (ticketId, cantidad) => {
    setCantidadBoletos(prev => ({
      ...prev,
      [ticketId]: cantidad,
    }));
  };
              ...
              {buscarPasajes().length > 0 ? (
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {buscarPasajes().map((ticket, index) => (
                    <div key={index} className="p-4 border rounded shadow bg-gray-100 flex flex-col">
                      ...
                      <div className="mt-2">
                        ...
                        <select
                          ...
                          onChange={(e) => handleCantidadChange(ticket.id, parseInt(e.target.value))}
                        >
                          {[...Array(10).keys()].map(num => (
                            <option key={num + 1} value={num + 1}>{num + 1} boleto(s)</option>
                          ))}
                        </select>
                      </div>

                      <button onClick={() => handleCompra(ticket)} className="mt-2 w-full bg-green-600 text-white p-2 rounded">
                        Comprar Pasaje
                      </button>
                    </div>
                  ))}
