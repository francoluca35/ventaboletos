'use client'
"use client"; // Importante para manejar el estado en Next.js 13+
...
export default function Home() {
  ...
  // Obtener la ubicación del usuario automáticamente
  useEffect(() => {
  ...
  // Obtener los destinos desde el JSON (Mock API en local)
  useEffect(() => {
    ...
    axios.get("http://localhost:5000/api/tickets")
      .then(response => {
        const resultados = response.data.filter(ticket =>
          ...
          (!fechaDesde || new Date(ticket.fecha) >= new Date(fechaDesde)) &&
          (!fechaHasta || new Date(ticket.fecha) <= new Date(fechaHasta))
        );
        ...
        <div className="mt-6 w-full max-w-3xl">
          {pasajes.length > 0 ? (
            <ul className="bg-white p-4 rounded-lg shadow-lg">
              {pasajes.map(ticket => (
                <li key={ticket.id} className="border-b py-2 flex justify-between">
                  <span>{ticket.destino} - ${ticket.precio}</span>
